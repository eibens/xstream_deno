...
export interface Operator<T, R>
  extends InternalProducer<R>, InternalListener<T>, OutSender<R> {
  type: string;
  ins: Stream<T>;
...
class StreamSub<T> implements Subscription {
  constructor(
    ...
    private _listener: InternalListener<T>,
  ) {}
...
export class Stream<T> implements InternalListener<T> {
  ...
  static fromObservable<T>(obs: { subscribe: any }): Stream<T> {
    ...
    const o = typeof obs[$$observable as never] === "function"
      ? (obs[$$observable as never] as () => never)()
      : obs;
