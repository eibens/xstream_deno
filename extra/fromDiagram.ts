// deno-lint-ignore-file no-prototype-builtins no-explicit-any no-case-declarations ban-types
  ...
  _start(out: InternalListener<any>) {
    ...
    for (let i = 0; i < L; i++) {
      ...
      switch (c) {
        ...
        case "#":
          this.schedule(
            { type: "error", value: this.errorVal, time: time },
            out,
          );
          break;
        case "|":
          ...
          break;
        default:
          const val = this.values.hasOwnProperty(c)
            ? (this.values as any)[c as any]
            : c;
 ...
 * @return {Stream}
 */
export default function fromDiagram(
  diagram: string,
