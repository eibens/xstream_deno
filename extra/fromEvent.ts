// deno-lint-ignore-file no-explicit-any no-inferrable-types ban-types
import { InternalListener, InternalProducer, Stream } from "../mod.ts";
...
export class NodeEventProducer implements InternalProducer<any> {
  ...
  constructor(private node: EventTarget, private eventName: string) {}

  _start(out: InternalListener<any>) {
    this.listener = (...args: Array<any>) => {
      return (args.length > 1) ? out._n(args) : out._n(args[0]);
    };
    this.node.addEventListener(this.eventName, this.listener as never);
  }

  _stop() {
    this.node.removeEventListener(this.eventName, this.listener as any);
    this.listener = null;
  }
}

function isEmitter(element: any): element is EventTarget {
  return element.emit && element.addListener;
}

function fromEvent<T = any>(element: EventTarget, eventName: string): Stream<T>;
...
function fromEvent<T = any>(
  ...
  if (isEmitter(element)) {
    ...
    return new Stream<T>(
      new DOMEventProducer(element, eventName, useCapture) as any,
    );
  }
}
