...
class FCAMIL<T> implements InternalListener<T>, OutSender<T> {
  constructor(public out: Stream<T>, private op: FlattenConcAMOperator<T>) {
  }
 ...
 * @return {Stream}
 */
export default function flattenConcurrentlyAtMost<T>(
  n: number,
): (ins: Stream<Stream<T> | MemoryStream<T>>) => Stream<T> {
  return function flattenConcAMOperator(
    ins: Stream<Stream<T> | MemoryStream<T>>,
  ) {
